...
def compute_pi_trends(all_papers):
    """Return dict: pi_name -> {count, avg_score, trend_label}."""
    from collections import defaultdict
    pi_papers = defaultdict(list)
    for p in all_papers:
        pi = p.get("pi", "").strip()
        if pi:
            pi_papers[pi].append(p["score"])
    trends = {}
    for pi, scores in pi_papers.items():
        avg = sum(scores) / len(scores)
        if len(scores) >= 3:
            recent = sum(scores[:2]) / 2      # sheet sorted by score desc
            ...
            trend = None
        trends[pi] = {"count": len(scores), "avg": round(avg, 1), "trend": trend}
    return trends


def next_action(score):
    """Rule-based suggested next action based on commercialization score."""
    if score >= 38:
        return "Request meeting with PI — strong commercialization potential"
    if score >= 30:
        return "Search patent landscape and assess freedom-to-operate"
    if score >= 22:
        return "Monitor for follow-up publications or grant activity"
    return "Low priority — flag for quarterly review"
...
ENRICHMENT_PROMPT = """You are a technology-transfer analyst. For the research paper below,
use web search to find real, specific examples. Return a JSON object (no markdown) with:
- comparable_deal: One real comparable licensing deal, acquisition, or investment in this
  technology area. Include company names and approximate value if known. Be specific.
  If nothing relevant found, write "No direct comparable found".
- competitor_scan: 1-2 sentences on competing companies or startups commercializing similar
  technology. Include names. If none found, write "No direct competitors identified".

Paper title: {title}
Research area: {fields}
Commercial angle: {opportunity}
"""


def enrich_paper_with_search(paper):
    """Run a grounded Gemini search to find comparable deals and competitor info."""
    from google.genai import types
    prompt = ENRICHMENT_PROMPT.format(
        title=paper["title"],
        fields=", ".join(paper.get("fields", [])),
        opportunity=paper.get("opportunity", ""),
    )
    try:
        ...
        # Extract JSON even if grounding adds extra text
        m = re.search(r'\{.*\}', text, re.DOTALL)
        if m:
            return json.loads(m.group())
    except Exception as e:
        print(f"    Enrichment search failed: {e}")
    return {"comparable_deal": "", "competitor_scan": ""}
...
def build_styles():
    ...
    return {
        ...
        "action": ParagraphStyle(
            ...
            fontSize=8, leading=11, textColor=C_GREEN,
            fontName="Helvetica-Bold", spaceAfter=2,
        ),
        "trend": ParagraphStyle(
            "trend", parent=base["Normal"],
            fontSize=8, leading=11, textColor=C_PURPLE2,
...
def paper_block(idx, paper, curation_item, styles, pi_trends=None, enrichment=None):
    ...
    if pi:
        ...
        if pi_trends and pi in pi_trends:
            t = pi_trends[pi]
            trend_str = f"  {t['count']} paper{'s' if t['count']>1 else ''} in system · avg score {t['avg']}"
            if t["trend"]:
                arrow = "↑" if t["trend"] == "trending up" else ("↓" if t["trend"] == "trending down" else "→")
                trend_str += f" · {arrow} {t['trend']}"
            inner.append(Paragraph(trend_str, styles["trend"]))
    ...
    # Comparable deal
    if enrichment and enrichment.get("comparable_deal") and \
            enrichment["comparable_deal"] != "No direct comparable found":
        inner.append(Paragraph("COMPARABLE DEAL", styles["label"]))
        inner.append(Paragraph(enrichment["comparable_deal"], styles["body"]))

    # Competitor scan
    if enrichment and enrichment.get("competitor_scan") and \
            enrichment["competitor_scan"] != "No direct competitors identified":
        inner.append(Paragraph("COMPETITOR SCAN", styles["label"]))
        inner.append(Paragraph(enrichment["competitor_scan"], styles["body"]))

    # Suggested next action
    action = next_action(paper["score"])
    inner.append(Paragraph("SUGGESTED ACTION", styles["label"]))
    inner.append(Paragraph(f"→ {action}", styles["action"]))
...
def generate_pdf(papers_by_idx, curation, monthly=False, branch=None,
                 pi_trends=None, enrichments=None):
    ...
    if monthly:
        ...
        output_pdf = DIGESTS_DIR / f"HUJI_digest_{year}_M{month:02d}{branch_suffix}_enhanced.pdf"
        ...
        week = iso[1]
        output_pdf = DIGESTS_DIR / f"HUJI_digest_{year}_W{week:02d}{branch_suffix}_enhanced.pdf"
    ...
    for rank, item in enumerate(curation["selected"], start=1):
        ...
        enrichment = (enrichments or {}).get(idx)
        story.extend(paper_block(rank, paper, item, styles,
                                 pi_trends=pi_trends, enrichment=enrichment))
...
def main(monthly=False):
    ...
    # Load all papers once for PI trend analysis
    print("Loading all papers for PI trend analysis...")
    try:
        all_papers = load_all_papers_from_sheet()
        pi_trends = compute_pi_trends(all_papers)
        print(f"  {len(pi_trends)} unique PIs found across {len(all_papers)} papers.")
    except Exception as e:
        print(f"  PI trend load failed: {e} — continuing without trends.")
        pi_trends = {}
    ...
    for branch, label in branch_cases:
        ...
        # Enrich each selected paper with grounded web search
        print("Enriching selected papers with web search...")
        enrichments = {}
        for item in selected:
            idx = item["index"] - 1
            if 0 <= idx < len(papers):
                paper = papers[idx]
                print(f"  Searching for: {paper['title'][:60]}...")
                enrichments[idx] = enrich_paper_with_search(paper)

        print("Generating HTML + PDF reports...")
        yr.generate_reports(
            papers, curation, monthly=monthly, branch=branch,
            is_fallback=is_fallback, out_dir=DIGESTS_DIR, variant="_enhanced",
