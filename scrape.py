...
* Visits each landing page in SITES
* Finds every link ending in .pdf / .doc / .docx
* Downloads new files into data/<sha1>.pdf
* Extracts “Issued / Deadline” dates from first page
* Appends / updates latest_rfps.json  (for GPT ingestion)

Run locally:  python scrape.py
Used in CI:   GitHub Action runs this file on a schedule
"""
...
def build_session(retries: int = 3, backoff: int = 2, timeout: int = 30) -> requests.Session:
    ...
    sess.request_timeout = timeout        # custom attribute for easy reuse
    return sess
...
def doc_links(url: str):
    """
    Yield absolute URLs of PDF / Word docs found on the landing page.
    Network or HTTP errors are logged and swallowed.
    """
    try:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if href.lower().endswith((".pdf", ".doc", ".docx")):
            yield href if href.startswith("http") else requests.compat.urljoin(url, href)
...
def parse_pdf(path: pathlib.Path) -> dict[str, str]:
    """Return dict with posted / deadline dates and a 5-line snippet."""
    try:
...
def main() -> None:
    ...
    # merge with previous JSON so history is kept
    if JSON_PATH.exists():
